...
double FeedFowardNeuralNetwork::determineRate(vector<vector<vector<double> > >& currGradients,
		vector<double> conjugateDirection, int i, int j, vector<double> directInputs) {
	vector<double> inputs;
	if (i-1 == -1) {
		inputs = directInputs;
	} else {
		for (int k = 0; k < nodes.at(i-1).size(); k++) {
			inputs.push_back(memorizedData.at(i-1).at(k).second);
		}
	}
	double norm = helper.dotProduct(inputs, inputs);
	vector<vector<double> > intermediateMatrix = helper.getIdentityMatrix(conjugateDirection.size());
	intermediateMatrix = helper.matrixReduce(intermediateMatrix, norm);
	return stepSize * nodes.at(i).at(j).determineRate(currGradients.at(i).at(j),
			conjugateDirection,
			intermediateMatrix);
}
			...
			for (int i = 0; i < nodes.size(); i++) {
				for (int j = 0; j < nodes.at(i).size(); j++) {
					...
					rates.at(i).at(j) = determineRate(gradients,
							conjugateDirection, i, j, inputs.at(z));
...
int main() {
	FeedFowardNeuralNetwork network = FeedFowardNeuralNetwork(20);
	...
	network.newtonBackPropagate(helper.boxVector(input), helper.boxVector(vector<double>(1, 0.7)), 3);
